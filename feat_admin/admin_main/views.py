...
def renting(request):
    if request.method == 'GET':
        ...
        user_history_query = Renting.objects.filter().only('userName', 'equipID_no',
                                                           'rentingDate', 'returningDate')  # Renting 테이블에서 원하는 컬럼만 가져온다.
        rent_user_list = []
        rent_list = []
...
def rent_detail_ajax(request):
    if request.method == 'POST':
        ...
        # 유저이름과 장비코드를 사용해 유저 아이디를 찾는다.
        user_rented_query = Renting.objects.get(
            userName=select_userName, equipID_no=select_equipID)
        select_equipID = user_rented_query.userID_no
        user_detail_query = User.objects.get(
            studentID__contains=select_equipID)
...
def returning(request):
    if request.method == 'GET':
        ...
        for data in user_history_query:
            # if data.rentingDate.strftime("%Y-%m-%d") >= now_date.strftime("%Y-%m-%d") : # 오늘 날짜 이후인 것만 보여준다.
            return_user_list.append(data)
        for data in return_user_list:
            equip_query = Equip.objects.filter(equipID=data.equipID_no)
            if equip_query:
                return_list.append(data)
                if str(data.equipID_no) == str(equip_query[0].equipID):
                    data.equipInfo = equip_query[0].equipInfo
        context = {'return_user_list': return_list}
        return render(request, 'admin_equipment_return.html', context)
    # return render(request, 'admin_equipment_rental.html')


@csrf_exempt
def return_detail_ajax(request):
    if request.method == 'POST':
        select_equipID = request.POST.get('select_equipID')
        select_userName = request.POST.get('select_userName')
        select_rentingDate = request.POST.get('select_rentingDate')
...
# *@csrf_exempt
# def return_ajax(request):
#   equipid = request.POST['select_equipID']
