...
from admin_main.models import Equip
...
def renting(request):
    if request.method == 'GET':
        ...
        for data in rent_user_list:
            equip_query = Equip.objects.get(equipID__contains=data.equipID)
            if str(data.equipID) == str(equip_query.equipID):
                data.equipInfo = equip_query.equipInfo
...
def rent_detail_ajax(request):
    if request.method == 'POST':
        ...
        user_rented_query = Renting.objects.get(userName=select_userName,equipID=select_equipID) #유저이름과 장비코드를 사용해 유저 아이디를 찾는다.
        select_equipID = user_rented_query.userID_id
